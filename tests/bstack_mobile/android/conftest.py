...
def driver(request):
    ...
    options = UiAutomator2Options().load_capabilities(common_options)
    options.set_capability(name='platformVersion', value=config.ANDROID_PLATFORM_VERSION)
    options.set_capability(name='deviceName', value=config.ANDROID_DEVICE_NAME)
